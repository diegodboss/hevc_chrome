...
├── public/
│   ├── ll-video.js             # WASM 解码器自定义元素（核心，运行 download-decoders.mjs 后生成）
│   └── decoder/                # WASM 解码器文件（运行 download-decoders.mjs 后生成）
│       ├── h264/               #   H.264 解码器
...
首次使用需要下载解码器文件到 `src/renderer/public/`：
...
```bash
node scripts/generate-hevc-hls.mjs [输出目录] [时长秒数]
# 例：node scripts/generate-hevc-hls.mjs test-streams/hevc-hls 30
```
